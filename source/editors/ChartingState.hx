...
class ChartingState extends MusicBeatState
{
	...
	public var boner:FlxSprite;
	public var movingwindow:FlxTimer;
	var debugIsOn = false;
	public var youthought:FlxSound;
	var undos = [];
	var redos = [];
	...
	[
		...
		['Camera Zoom Tween', "This zooms in on the current character\nValue 1: How much the camera should zoom in\nValue 2: How fast the camera should zoom in"],
		...
		['BetrayAttack',"Sans betrays your trust"],
		['SansAttack',"Normal sans attack"],
		['attackWarning',"Primarily used for warning when Hungry Sans is about to attack"],
		['HungryAttack',"He's been starving for 7 years. And you look TASTY"],
		['BoneSurpriseAttack',"Flowey did say he's unpredictable"],
		['TransitionAttack',"You dodged right into his axe. Fucking idiot"],
		['TheComedian',"What's wrong? You need a 'hand' there, pal?"],
		['GoingCrazyNow',"Fanon horror!sans would act a lot more insane than this one"],
		['CharacterChangedAndItRemovedTheColorSoThisIsMySolution',''],
		['BackToSemi-Calm',''],
		['End',''],
		['AlizaGetsKilled',''],
		['Set Fear Mult', "Value 1: New Value\n\nAdds the typed value to the current fear multiplier."],
		['cinematic',"makeGraphic didn't work in lua, so I did added another event instead."]
	];
	...
	**/
	...
	override function create()
	{
		...
		//What made you think I was going to let you use the chart editor, human?
		boner = new FlxSprite(-300,400).loadGraphic(Paths.image('Bones'));
		add(boner);
		boner.antialiasing = true;
		boner.setGraphicSize(Std.int(boner.width * 1.25),Std.int(boner.height * 1.25));
		new FlxTimer().start(.5, function(tmr:FlxTimer){
			FlxG.camera.shake(0.1,5,null,true,FlxAxes.X);
			FlxTween.tween(boner, {y:-400}, .1,{onComplete: function(twn:FlxTween) {
			...
			var no:FlxSound;
			no = new FlxSound().loadEmbedded(Paths.sound('fuckinidiot'));
			no.play(true);
			new FlxTimer().start(.01, function(tmr:FlxTimer){
				...
				var xVal3 = Std.int(windowX - 20);
				var yVal3 = Std.int(windowY);
				Lib.application.window.move(xVal3,yVal3);
			movingwindow = new FlxTimer().start(.01, function(tmr:FlxTimer){
				var xVal = Std.int(windowX + 20);
				var yVal = Std.int(windowY);
				Lib.application.window.move(xVal,yVal);
				new FlxTimer().start(.01, function(tmr:FlxTimer){
					var xVal2 = Std.int(windowX - 20);
					var yVal2 = Std.int(windowY);
					Lib.application.window.move(xVal2,yVal2);
					movingwindow.reset(.01);
				});
			});
		});
		});
